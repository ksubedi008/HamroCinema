...
const OpenGraphPreview = ({ title, genre, description, poster }) => {
    ...
    return (
        <div className="mt-6 flex flex-col space-y-3 p-4 rounded-2xl bg-black/20 border border-zinc-800">
            <div className="flex items-center gap-2">
                <svg className="w-5 h-5 text-rose-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1" />
                </svg>
                <label className="text-sm font-bold tracking-wider uppercase text-rose-500">Social Share Preview</label>
            </div>
            
            <div className="w-full mx-auto overflow-hidden rounded-xl bg-zinc-800  border border-zinc-800 shadow-lg transition-all">
                {/* 1.91:1 Aspect Ratio Image Container */}
                <div className="w-full aspect-[1.91/1] bg-zinc-950 relative border-b border-zinc-800 flex items-center justify-center overflow-hidden">
                    {posterUrl ? (
                    ...
                    {genre && (
                        <div className="absolute top-3 right-3 px-2.5 py-1 bg-zinc-900/90  rounded-lg border border-zinc-800 text-[10px] font-bold text-white tracking-widest uppercase">
                            {genre}
                        </div>
                    )}
                </div>
