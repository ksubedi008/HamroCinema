...
const CustomerLayout = () => { 
  ...
  // Guests can also browse the public site normally. 
  ...
  return ( 
    <div className="min-h-screen bg-gray-50 text-gray-900 dark:bg-[#121212] dark:text-neutral-100 flex flex-col font-sans relative overflow-hidden transition-colors duration-200 ease-in-out"> 
      <div className="bg-noise opacity-50 dark:opacity-100"></div> 
      <header className="bg-[#1A1A1A] border-b border-neutral-800 sticky top-0 z-50"> 
            ...
            <div className="hidden md:flex items-center gap-4"> 
              {user ? ( 
                <div className="relative" ref={dropdownRef}> 
                  <button onClick={() => setIsProfileOpen(!isProfileOpen)} className="flex items-center gap-3 pl-2 pr-4 py-1.5 rounded-full bg-[#121212] border border-neutral-800 hover:border-neutral-600 hover:shadow-xl transition-all duration-200 ease-in-out" > 
                    {user.profile_picture ? ( 
                      <img src={getProfileImageUrl(user?.profile_picture)} alt={user.username} className="w-8 h-8 object-cover rounded-full border border-zinc-700" /> 
                    ) : ( 
                      <div className="w-8 h-8 rounded-full bg-gradient-to-br from-purple-500 to-pink-500 flex items-center justify-center font-bold text-sm shadow-inner text-neutral-100"> {user.username[0].toUpperCase()} </div> 
                    )} 
                    <span className="text-sm font-bold text-gray-200">{user.username}</span> 
                    <svg className={`w-4 h-4 text-neutral-400 hover:text-neutral-100 transition-transform duration-200 ease-in-out ${isProfileOpen ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" /></svg> 
                  </button> 
                  
                  {isProfileOpen && ( 
                    <div className="absolute right-0 mt-4 w-56 bg-[#1A1A1A] border border-neutral-800 rounded-2xl overflow-hidden shadow-lg animate-fade-in z-50 transform origin-top-right transition-all duration-200 ease-in-out"> 
                      <div className="px-4 py-3 border-b border-neutral-800 bg-purple-900/10"> 
                      ...
                      <div className="py-2"> 
                        ...
                        {(user.role === 'Admin' || user.role === 'Manager') && ( 
                          <Link to="/k-subedi-08/dashboard" onClick={() => setIsProfileOpen(false)} className="flex items-center gap-3 px-4 py-2.5 text-sm font-bold bg-[#1A1A1A] hover:bg-zinc-700 text-neutral-400 hover:text-neutral-100 transition-colors duration-200 ease-in-out border-l-2 border-neutral-600"> 
                        ...
                        <Link to="/profile/password" onClick={() => setIsProfileOpen(false)} className="block px-4 py-2 text-sm font-medium text-neutral-300 hover:bg-gray-800 hover:text-neutral-100 transition-colors duration-200 ease-in-out"> Change Password </Link> 
                      </div> 
                      
                      <div className="py-2 border-t border-neutral-800"> 
                        <button onClick={() => { setIsProfileOpen(false); logoutUser(); }} className="w-full text-left block px-4 py-2 text-sm font-medium text-neutral-300 hover:bg-gray-800 hover:text-red-500 transition-colors duration-200 ease-in-out"> Logout </button> 
                      </div> 
                    </div> 
                  )} 
                </div> 
              ) : ( 
                <Link to="/login" className="btn-premium px-6 py-2.5 rounded-full text-sm"> Sign In </Link> 
              )} 
            </div> 
            
            {/* Mobile Hamburger Button */} 
            <div className="md:hidden flex items-center gap-3 z-50"> 
              <button onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)} className="text-neutral-300 hover:text-neutral-100 p-2 focus:outline-none"> 
              ...
              {user ? ( 
                <> 
                  <div className="flex items-center gap-3 py-2"> 
                    {user.profile_picture ? ( 
                      <img src={getProfileImageUrl(user?.profile_picture)} alt={user.username} className="w-10 h-10 object-cover rounded-full border border-zinc-700" /> 
                    ) : ( 
                      <div className="w-10 h-10 rounded-full bg-gradient-to-br from-purple-500 to-pink-500 flex items-center justify-center font-bold text-lg shadow-inner text-neutral-100"> {user.username[0].toUpperCase()} </div> 
                    )} 
                    <div> 
                      <p className="text-sm text-neutral-400">Signed in as</p> 
                      <p className="text-base font-bold text-neutral-100">{user.username}</p> 
                    </div> 
                  </div> 
                  
                  {/* Mobile VIP Command Center Link */} 
                  {(user.role === 'Admin' || user.role === 'Manager') && ( 
                    <Link to="/k-subedi-08/dashboard" onClick={() => setIsMobileMenuOpen(false)} className="flex items-center gap-2 text-base font-bold text-neutral-400 hover:text-neutral-100 py-2"> 
                  ...
                  <Link to="/profile/password" onClick={() => setIsMobileMenuOpen(false)} className="block text-base font-medium text-neutral-300 hover:text-neutral-100 py-2">Change Password</Link> 
                  <button onClick={() => { setIsMobileMenuOpen(false); logoutUser(); }} className="w-full text-left block text-base font-medium text-neutral-300 hover:text-red-500 py-2">Logout</button> 
                </> 
              ) : ( 
                <Link to="/login" onClick={() => setIsMobileMenuOpen(false)} className="btn-premium text-center py-3 rounded-full mt-2">Sign In</Link> 
              )} 
            </div> 
          </div> 
        )} 
      ...
      <footer className="border-t border-neutral-800 bg-[#121212] relative overflow-hidden"> 
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 relative z-10"> 
          <div className="grid grid-cols-1 md:grid-cols-4 gap-12 md:gap-8"> 
            ...
            <div className="col-span-1 md:col-span-2"> 
              ...
              <div className="inline-flex flex-col"> 
                <div className="flex flex-col gap-1"> 
                  ...
                  <p className="text-lg font-bold text-neutral-300 hover:text-neutral-400 hover:text-neutral-100 transition-colors duration-200 ease-in-out"> 
                    Kamal Subedi 
                  </p> 
                </div> 
              </div> 
            ...
            {/* Legal */} 
            <div> 
              <h3 className="text-neutral-100 font-bold mb-6 text-sm tracking-wider uppercase">Legal</h3> 
              <ul className="flex flex-col gap-4 text-sm text-neutral-400"> 
                <li><Link to="/terms" className="hover:text-neutral-400 hover:text-neutral-100 transition-colors duration-200 ease-in-out">Terms of Service</Link></li> 
                <li><Link to="/privacy-policy" className="hover:text-neutral-400 hover:text-neutral-100 transition-colors duration-200 ease-in-out">Privacy Policy</Link></li> 
                <li><Link to="/refund-policy" className="hover:text-neutral-400 hover:text-neutral-100 transition-colors duration-200 ease-in-out">Refund Policy</Link></li> 
                <li><Link to="/contact" className="hover:text-neutral-400 hover:text-neutral-100 transition-colors duration-200 ease-in-out">Contact Us</Link></li> 
              </ul> 
            </div> 
          </div> 
          ...
          <div className="mt-16 pt-8 border-t border-neutral-800 flex flex-col md:flex-row items-center justify-between gap-4"> 
            ...
            <div className="flex gap-4"> 
              {/* Instagram */} 
              <a href="https://www.instagram.com/k_subedi08/" target="_blank" rel="noopener noreferrer" aria-label="Instagram" className="w-8 h-8 rounded-full bg-[#1A1A1A] flex items-center justify-center hover:bg-pink-600/20 hover:border-neutral-800 cursor-pointer transition-all duration-200 ease-in-out border border-neutral-800 group"> 
                <svg className="w-4 h-4 text-neutral-400 group-hover:text-neutral-400 hover:text-neutral-100 transition-colors duration-200 ease-in-out" fill="currentColor" viewBox="0 0 24 24"> 
              ...
              {/* WhatsApp */} 
              <a href="https://wa.me/9779820523224" target="_blank" rel="noopener noreferrer" aria-label="WhatsApp" className="w-8 h-8 rounded-full bg-[#1A1A1A] flex items-center justify-center hover:bg-green-600/20 hover:border-green-500/50 cursor-pointer transition-all duration-200 ease-in-out border border-neutral-800 group"> 
                <svg className="w-4 h-4 text-neutral-400 group-hover:text-green-400 transition-colors duration-200 ease-in-out" fill="currentColor" viewBox="0 0 24 24"> 
