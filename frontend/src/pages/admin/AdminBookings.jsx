...
const AdminBookings = () => {
  ...
  return (
    <div className="animate-fade-in space-y-6">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
        ...
        <div>
          ...
          <p className="text-neutral-400 mt-1 pl-4">
            Monitor customer ticket purchases.
          </p>{" "}
        </div>{" "}
      ...
      <div className="bg-[#121212] border border-neutral-800 rounded-2xl overflow-x-auto w-full shadow-xl">
        {" "}
        <table className="w-full text-left text-sm text-neutral-300 whitespace-nowrap">
          ...
          <tbody className="divide-y divide-neutral-800">
            ...
            {filteredBookings.map((b) => (
              ...
              >
                {" "}
                <td className="px-6 py-4 font-mono text-neutral-400 hover:text-neutral-100">
                  #{b.id.toString().padStart(6, "0")}
                </td>{" "}
                <td className="px-6 py-4 font-medium text-neutral-100">
                  {b.username || `User ${b.user}`}
                </td>{" "}
                ...
                <td className="px-6 py-4">
                  {" "}
                  <span
                    className={`px-2 py-1 rounded text-xs font-bold ${b.status === "Completed" ? "bg-[#1A1A1A] text-green-400" : "bg-[#1A1A1A] text-yellow-400"}`}
                  >
                    {" "}
                    {b.status}{" "}
                  </span>{" "}
