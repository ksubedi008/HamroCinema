...
const AdminShowtimes = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const sortedShowtimes = showRes.data.sort(
  ...
  const displayShowtimes =
    activeTab === "upcoming"
      ...
      : historyShowtimes.filter((st) => {
          ...
          const titleMatch = (st.movie_title || `Movie ID: ${st.movie}`)
            .toLowerCase()
    ...
    try {
      ...
      for (const s of screenList) {
        for (const sh of shiftList) {
          requests.push(
            axios.post(`${import.meta.env.VITE_API_BASE_URL}/api/showtimes/`, {
      ...
      if (err.response && err.response.data) {
        ...
        alert(
          "Failed to schedule some showtimes. Please check for collisions.",
        );
      }
    } finally {
  ...
  const handleDelete = async (id) => {
    if (
      window.confirm(
        "Confirm cancellation of showtime? This action cannot be undone.",
      )
    ) {
      try {
        await axios.delete(
          `${import.meta.env.VITE_API_BASE_URL}/api/showtimes/${id}/`,
        );
        fetchData();
      } catch (err) {
        console.error(err);
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
        <div>
          ...
          <p className="text-neutral-400 mt-1 pl-4">
            Schedule and monitor all theater activity.
          </p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-4 w-full md:w-auto">
          <button
            ...
            className="btn-premium w-full sm:w-auto px-6 py-2 rounded-xl text-sm font-bold tracking-wider"
          >
            + Schedule Movie
          </button>
          ...
          <div className="flex bg-[#1A1A1A] p-1 rounded-xl border border-neutral-800">
            <button
              ...
              className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors duration-200 ease-in-out border ${activeTab === "upcoming" ? "bg-neutral-800 text-neutral-100 border-transparent hover:border-neutral-600 hover:text-neutral-100 shadow-sm" : "border-transparent text-neutral-400 hover:text-neutral-300"}`}
            >
              Upcoming ({upcomingShowtimes.length})
            </button>
            <button
              onClick={() => setActiveTab("history")}
              className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors duration-200 ease-in-out border ${activeTab === "history" ? "bg-neutral-800 text-neutral-100 border-transparent hover:border-neutral-600 hover:text-neutral-100 shadow-sm" : "border-transparent text-neutral-400 hover:text-neutral-300"}`}
            >
        ...
        <table className="w-full text-left text-sm text-neutral-300 whitespace-nowrap">
          ...
          <tbody className="divide-y divide-neutral-800">
            {displayShowtimes.map((st) => {
              ...
              if (now > stEnd)
                statusBadge = (
                  <span className="bg-[#1A1A1A] text-neutral-400 px-3 py-1 rounded-md text-xs font-bold border border-neutral-800 uppercase tracking-widest">
                    Ended
                  </span>
                );
              else if (now >= stStart && now <= stEnd)
                statusBadge = (
                  <span className="bg-[#1A1A1A] text-green-400 px-3 py-1 rounded-md text-xs font-bold border border-neutral-800 uppercase tracking-widest shadow-lg">
                    Playing
                  </span>
                );
              else
                statusBadge = (
                  <span className="bg-[#1A1A1A] text-neutral-400 hover:text-neutral-100 px-3 py-1 rounded-md text-xs font-bold border border-neutral-800 uppercase tracking-widest shadow-lg">
                    Upcoming
                  </span>
                );
              ...
              return (
                ...
                >
                  <td className="px-6 py-4 font-bold text-neutral-100">
                    {st.movie_title || `Movie ID: ${st.movie}`}
                  </td>
                  <td className="px-6 py-4 font-medium text-neutral-400 hover:text-neutral-100">
                    {st.screen_name || `Screen ID: ${st.screen}`}
                  </td>
                  <td className="px-6 py-4 text-neutral-300 font-medium">
                    {stStart.toLocaleDateString()}{" "}
                    <span className="text-neutral-400 mx-1">•</span>{" "}
                    {stStart.toLocaleTimeString([], {
                      hour: "2-digit",
                      minute: "2-digit",
                    })}
                  </td>
                  ...
                  <td className="px-6 py-4 text-right">
                    {now <= stStart && (
                      <button
                        ...
                        className="text-red-400 hover:text-red-300 font-bold px-3 py-1.5 rounded hover:bg-neutral-800 transition-colors duration-200 ease-in-out uppercase tracking-widest text-xs border border-transparent hover:border-neutral-800"
                      >
                        Cancel
                      </button>
                    )}
                ...
                >
                  <div className="flex flex-col items-center gap-3">
                    ...
                    {activeTab === "history" && searchQuery ? (
                      <p className="text-lg">
                        No historical showtimes found for{" "}
            ...
            {loading && (
              <tr>
                <td
                  colSpan="6"
                  className="px-6 py-16 text-center text-neutral-400 hover:text-neutral-100"
                >
                  <div className="flex flex-col items-center gap-3">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              <div className="space-y-1">
                ...
                <select
                  ...
                  className="w-full bg-[#121212] border border-neutral-800 rounded-xl px-4 py-2.5 text-neutral-100 focus:outline-none focus:border-neutral-600"
                >
              ...
              <div className="space-y-1">
                ...
                <input
                  ...
                  className="w-full bg-[#121212] border border-neutral-800 rounded-xl px-4 py-2.5 text-neutral-100 focus:outline-none focus:border-neutral-600 [color-scheme:dark]"
                />
              </div>
              <div className="space-y-1">
                <label className="text-sm font-medium text-neutral-400">
                ...
                <p className="text-xs text-neutral-400 mt-2 flex items-center gap-2">
                  ...
                  Start times, end times, and ticket prices will be
                  automatically assigned based on the shift selected.
                </p>
              </div>
              ...
              <div className="pt-4 flex justify-end gap-3">
                ...
                <button
                  ...
                  className="btn-premium px-5 py-2.5 rounded-xl flex items-center gap-2 disabled:opacity-70 disabled:cursor-not-allowed"
                >
                  {isSubmitting ? (
                    <>
                      <svg
