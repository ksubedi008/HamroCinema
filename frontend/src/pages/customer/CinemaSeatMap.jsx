...
const CinemaSeatMap = ({ seats, bookedTickets, selectedSeats, onSeatClick, getPrice, showtime }) => {
    ...
    const renderSeat = (row, col) => {
        ...
        if (isBooked) {
            seatClass += "bg-zinc-800 text-zinc-600 cursor-not-allowed border border-zinc-700";
        } else if (isSelected) {
            ...
            seatClass += "bg-transparent border border-gray-500 text-gray-300 hover:bg-zinc-800 hover:border-gray-400";
        }
    ...
    return (
        <div className="flex flex-col items-center">
            ...
            <div className="mb-16 relative w-full max-w-2xl mx-auto flex flex-col items-center">
                ...
                <p className="text-center text-gray-400 text-sm font-medium mt-2">Screen</p>
            </div>
            ...
            <div className="flex flex-wrap justify-center gap-6 md:gap-12 mt-12 pt-8 border-t border-zinc-800/50 w-full">
                <div className="flex items-center gap-3">
                    ...
                    <span className="text-sm text-gray-400 font-medium tracking-wide">Premium (Rs. 500)</span>
                </div>
                <div className="flex items-center gap-3">
                    <div className="w-6 h-6 bg-transparent border border-gray-500 rounded-t-md"></div>
                    <span className="text-sm text-gray-400 font-medium tracking-wide">Standard (Rs. 300)</span>
                </div>
                <div className="flex items-center gap-3">
                    <div className="w-6 h-6 bg-emerald-500 border border-emerald-500 rounded-t-md"></div>
                    <span className="text-sm text-gray-400 font-medium tracking-wide">Selected</span>
                </div>
                <div className="flex items-center gap-3">
                    <div className="w-6 h-6 bg-zinc-800 border border-zinc-700 rounded-t-md"></div>
                    <span className="text-sm text-gray-400 font-medium tracking-wide">Booked</span>
                </div>
            </div>
        </div>
    );
