...
const SeatSelection = () => { 
  ...
  const getPrice = (seat) => { 
    ...
    // Middle block (M) 
    return 300; 
    // Left/Right block (L/R) 
  }; 
  
  const handleSeatClick = (seat) => { 
    if (bookedTickets.some(t => t.seat.toString() === seat.id.toString())) return; 
    // Booked 
  ...
  return ( 
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5 }} className="w-full pb-20 min-h-screen pt-8 relative overflow-hidden" > 
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10"> 
        ...
        <div className="flex items-center justify-between mb-8 pb-6 border-b border-neutral-800"> 
          <div> 
            <h1 className="text-3xl font-black text-neutral-100 mb-2">{showtime.movie_title}</h1> 
        ...
        <div className="flex flex-col lg:flex-row gap-12"> 
          {/* Cinema Room Canvas */} 
          <div className="flex-1 bg-[#121212] rounded-3xl p-4 md:p-8 border border-neutral-800 shadow-lg"> 
            <CinemaSeatMap seats={seats} bookedTickets={bookedTickets} selectedSeats={selectedSeats} onSeatClick={handleSeatClick} getPrice={getPrice} showtime={showtime} /> 
          </div> 
          {/* Summary Sidebar */} 
            ...
            <div className="bg-[#121212] rounded-3xl p-6 border border-neutral-800 sticky top-24"> 
              ...
              {selectedSeats.length === 0 ? ( 
                <div className="text-center py-10"> 
                  ...
                  <p className="text-neutral-400 text-sm">Please select your seats to see the summary.</p> 
                </div> 
              ) : ( 
                <> 
                  <div className="space-y-3 mb-6 max-h-60 overflow-y-auto pr-2 custom-scrollbar"> 
                    {selectedSeats.map(seat => ( 
                      <div key={seat.id} className="flex justify-between items-center bg-[#121212] p-3 rounded-xl border border-neutral-800"> 
                        <div> 
                          <p className="text-neutral-100 font-bold">{seat.seat_label}</p> 
                          <p className="text-xs text-neutral-400 hover:text-neutral-100">{seat.tier}</p> 
                        </div> 
                        <p className="text-neutral-100 font-medium">Rs. {getPrice(seat)}</p> 
                      </div> 
                    ))} 
                  </div> 
                  <div className="border-t border-neutral-800 pt-4 mb-6"> 
                    ...
                    <div className="flex justify-between items-center text-xl font-black text-neutral-100"> 
                      <span>Total</span> 
                      <span className="text-neutral-400 hover:text-neutral-100">Rs. {totalPrice}</span> 
                    </div> 
                  </div> 
                  {showtime && (new Date(showtime.start_time).getTime() - (15 * 60 * 1000) <= Date.now()) ? ( 
                    <button disabled className="w-full py-4 rounded-xl flex items-center justify-center gap-2 bg-[#1A1A1A] text-neutral-400 font-bold border border-neutral-800 cursor-not-allowed uppercase tracking-widest text-sm" > Booking Closed </button> 
                  ) : ( 
                    <button onClick={handleProceed} className="btn-premium w-full py-4 rounded-xl flex items-center justify-center gap-2" > 
